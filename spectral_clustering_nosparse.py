...
"""
===============================================================================
1.spectral_clustering_nosparse
    And the adjacency matrix of the graph,
    from the dot product of the information given in the direction of that from
    vertex to vertex, perform the classification by K-Means.
===============================================================================
...
# Custom Library
#import kernel_matrix as km # Create a kernel matrix using a Gaussian kernel
# function import dualkmeans as dkm   # Based on the duality matrices,
# according to the k-means then the classification

# =============================================================================
# spectral_clustgering
#    And the adjacency matrix of the graph,
#    from the dot product of the information given in the direction of that
#    from vertex to vertex, perform the classification by K-Means.
# =============================================================================
# *** Arguments ***
#   K  : kernel matrix
...
# =============================================================================
def spectral_clustering_nosparse(K,c,kmf,ptf,ds=False):
    ...
    # -------------------------------------------------------------------------
    # The invitation by the eigenvalue decomposition, the eigenvalues ​​and
    # eigenvectors
    # -------------------------------------------------------------------------
    # If the value of a sparse, eigenvalue decomposition of scipy only handles
    # up to vector that is perpendicular to it and eigenvalues ​​of maximum.
    # Therefore, only two points are extracted on circle.
    # -------------------------------------------------------------------------
    #ev,ed = np.linalg.eig(K) # returns eigen-values, eigen-vectors
    #print "ev-1\n", (ev)
    #print "ed-1\n", (ed)
    ...
    # Locate the lowest eigenvalue, and then compressed to a dimension that
    # the eigenvectors
    md = min(abs(ev))
    mi = aindex(ev, md, len(ev))
    if ds == True:
        mi = c if c < mi else mi
        print("axis-num:%d->%d" % (mi, c))
    ed = ed[:,0:mi]
    print "aindex"
    ...
    #savetxt("rand.dat", [ceil(np.random.random() * N) for i in range(R)], \
    #    fmt="%d")
...
#   Therefore, it returns the number of the array of elements with the value of
#   the minimum
# =============================================================================
# *** Arguments ***
...
#   I get the results of the classification of spectral clustering.
# =============================================================================
# *** Arguments ***
#   K:    source matrix(graph)
#   N:    eigenvalue
#   R:    eigenvectors
#   name: result file name
# =============================================================================
def create_classification(K, R, mi, name):
    R = R.real
    print "R\n", R
    ...
    savetxt(name, RI.real, fmt="%d")
    print("graph saved(cluster-num=%d)" % (C))
...
# =============================================================================
# Reads the data file
# Data file is a newline character to separate the code that depends on the OS 
# space and given x, y, label {0,1}
# =============================================================================
# -----------------------------------------------------------------------------
# Initial processing
# -----------------------------------------------------------------------------
...
#R,mi = spectral_clustering_nosparse(K,C,True,True)
#C,N  = create_classification(K, R, mi, "result_spectral_clustering.txt")

#draw_classification(R, C, N)
